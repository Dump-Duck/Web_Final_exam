...
<body>
    ...
    <div class="update-song">
        ...
        ?>
                <form class='insert-form' method="post">
                    <p id='input-form'><input type="hidden" name="id" value='<?php echo $id; ?>'></p>
                    <p id='input-form'>Title: <input type="text" name="title" value='<?php echo $title; ?>'></p>
                    <p id='input-form'>Artist: <input type="text" name="artist" value='<?php echo $artist; ?>'></p>
                    <p id='input-form'>Genre: <?php
                                                $genre_sql = "SELECT * FROM genres WHERE id = '" .  $genre . "'";
                    ...
                    <p id='input-form'>National: <?php
                                                    $national_sql = "SELECT * FROM nationals WHERE id = '" . $national . "'";
                                                    $national_query = $conn->query($national_sql);
                                                    while ($national_row = $national_query->fetch_assoc()) {
                                                        echo $national_row['national'];
                                                    }
                                                    ?></p>
                    <p id='input-form'>Image: <?php echo "<img src='Images/song/$image' width='250px' height='250px'>"; ?></p>
                    <hr style='border: 2px solid #3c2d57'>
                    <p id="button"><input type="submit" name="update" value="Update"><button style='margin-left:10px'><a href='song_manage.php' style='color:black'>Cancel</a></button></p>
                </form>
        <?php
            }
        }
    ...
    if (isset($_POST['update'])) {
        ...
        if ($conn->query($sql) == TRUE) {
            echo "<script>alert('Update song successfully! You will be back to song management page after click OK!');</script>";
            header("refresh:0.1;url=song_manage.php");
        }
        $conn->close();
    }
    ?>
